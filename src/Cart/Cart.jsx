...
function Cart() {
    const {items,totalItems,totalUniqueItems,cartTotal,updateItemQuantity,removeItem,emptyCart} = useCart();
                ...
                <section className="shopping-cart">
                    <div className="container">
                        <div className="text-center">
                            <div style={{fontSize: '4rem', color: '#ddd', marginBottom: '15px'}}>
                                🎮
                            </div>
                            <h2 style={{color: '#0071f8', marginBottom: '15px'}}>Your Gaming Cart is Empty</h2>
  ...
  <div className="container">
    <div className="text-center mb-5">
      <h2 style={{color: '#0071f8', fontSize: '2.5rem', fontWeight: 'bold'}}>
        🛒 Your Gaming Cart
      </h2>
      <p style={{color: '#666', fontSize: '1.1rem'}}>
        {totalUniqueItems} game{totalUniqueItems !== 1 ? 's' : ''} selected • {totalItems} total items
      </p>
    </div>
      ...
      <div className="row">
        <div className="col-md-12 col-lg-8">
          <div className="items">
            ...
            {
                return(
            <div className="product" key={item.id} style={cartStyles.productCard}>
                    ...
                    <div className="row align-items-center">
                      <div className="col-md-5">
                        <div className="product-name">
                          <h5 style={{color: '#333', fontWeight: 'bold', marginBottom: '10px'}}>
                            {item.title}
                          </h5>
                          <div className="product-info">
                            <div className="mb-1">
                              <span className="badge" style={{backgroundColor: '#ee626b', color: 'white', marginRight: '5px'}}>
                                {item.category}
                              </span>
                              {item.genre && (
                                <span className="badge bg-secondary">{item.genre}</span>
                              )}
                            </div>
                      ...
                      <div className="col-md-4 text-center">
                        <label className="form-label fw-bold" style={{color: '#333'}}>Quantity</label>
                        <div className="d-flex align-items-center justify-content-center mb-2">
                          <span className="mx-3 fw-bold fs-5" style={{color: '#0071f8'}}>
                            {item.quantity}
                          </span>
                        </div>
                        ...
                        <div className="price-section">
                          {item.originalPrice && item.originalPrice > item.price && item.price > 0 && (
                            <small style={cartStyles.discountPrice}>
                              ${Math.ceil(item.originalPrice * item.quantity)}
                            </small>
                          )}
                          ...
                          {item.originalPrice && item.originalPrice > item.price && item.price > 0 && (
                            <small className="text-success">
                              Save ${Math.ceil((item.originalPrice - item.price) * item.quantity)}
                            </small>
                          )}
                        </div>
                      </div>
        ...
        <div className="col-md-12 col-lg-4">
          <div className="summary" style={cartStyles.summaryCard}>
            ...
            <div className="summary-item d-flex justify-content-between mb-3" style={{borderBottom: '1px solid rgba(255,255,255,0.2)', paddingBottom: '10px'}}>
              <span className="text-white-50">Games ({totalItems})</span>
              <span className="text-white fw-bold">${Math.ceil(cartTotal)}</span>
            </div>
            <div className="summary-item d-flex justify-content-between mb-3" style={{borderBottom: '1px solid rgba(255,255,255,0.2)', paddingBottom: '10px'}}>
              <span className="text-white-50">Gaming Discount</span>
              <span className="text-warning fw-bold">-$5</span>
            </div>
            <div className="summary-item d-flex justify-content-between mb-4" style={{borderBottom: '1px solid rgba(255,255,255,0.2)', paddingBottom: '10px'}}>
            ...
            <div className="summary-item d-flex justify-content-between mb-4">
              ...
              <span className="text-white fw-bold fs-4">${Math.ceil(cartTotal - 5)}</span>
            </div>
            <button 
              type="button" 
